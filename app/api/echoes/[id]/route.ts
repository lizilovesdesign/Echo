...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  try {
    ...
    const updated = await echoEntryRepository.update(entryId, {
      ...
      entryType: (input.entryType ?? entry.entryType) as 'song' | 'album',
      albumName: input.albumName ?? entry.albumName ?? undefined,
      spotifyAlbumId: input.spotifyAlbumId ?? entry.spotifyAlbumId ?? undefined,
