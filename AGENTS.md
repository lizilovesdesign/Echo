...
## Tech Stack
- **Frontend (Web/Mobile):** Next.js (App Router) and React Native via the Expo framework
...
└── workflows/                         (step-by-step recipes for common tasks)
    ...
    └── new-mobile-screen.md           ← Adding an Expo mobile screen
```
...
3. **Web-First Logic Sharing:** Core business models, validation logic, and utility functions must reside in a centralized, shared space to keep Next.js Route Handlers and React Native (Expo) completely synchronized.
4. **Prisma Lifecycle Safety:** The Prisma Client must be instantiated as a strict singleton pattern to completely block connection pool exhaustion across serverless environments.
5. **Secrets Security:** Sensitive credentials (Spotify API client keys, Supabase URLs, database string endpoints) reside exclusively in env variables and must never be exposed or committed to the codebase.
